...
public class AudioReceiveBot : Gtk.Window {
  ...
  construct {
    ...
    try {
      pipeline = (Gst.Pipeline) Gst.parse_launch("appsrc name=src !"
                                              + " tee name=t ! queue ! audioresample ! audioconvert ! autoaudiosink"
                                              + " t. ! queue ! audioconvert ! wavescope ! videoconvert ! gtksink name=gtksink");
  ...
  ~AudioReceiveBot() {
    stdout.printf("[LOG] AudioReceiveBot destructed.\n");
  }
  ...
  private static void on_self_connection_status(ToxCore.Tox self, ToxCore.Connection connection, void* user_data) {
    AudioReceiveBot _this = (AudioReceiveBot) user_data;
  ...
  public static int main (string[] args) {
    ...
    var sample = new AudioReceiveBot();
    sample.show_all();
